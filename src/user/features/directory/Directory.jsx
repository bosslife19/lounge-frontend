...
const Directory = () => {
  ...
  const dropdownOptions = [
    { text: "Settings", icon: FaCog, handler: () => navigate("/settings") },
    {
      text: "Logout",
      icon: HiOutlineLogout,
  ...
  useEffect(() => {
    ...
    if (locationSearch) {
      ...
      results = results.filter((item) =>
        [item.city, item.organization?.location]
          .filter(Boolean)
          .some((f) => f.toLowerCase().includes(lowerLoc))
      );
    }
    setFilteredResults(results);
  }, [search, locationSearch, directoryData]);

  const handleSelect = (card) => {
    setSelected(card);
    if (window.innerWidth < 768) setMobileModalOpen(true); // mobile only
  };
    ...
    <Box w="100%">
      ...
      >
              ...
              >
                ...
                <HStack mt={3}>
                  ...
                  <Stack mt={-2}>
                    ...
                    <HStack mt={-2} alignItems="center">
                      <FaLocationDot fontSize="10px" color="#7C7C7C" />
                      <Text
                        color="#7C7C7C"
                        fontSize={{ base: "10px", xl: 12 }}
                        fontFamily="InterRegular"
                      >
                        {card.organization?.location || ""}
                      </Text>
        ...
        <Box
          ...
          boxShadow="md"
        >
          {selected && <UserDetails selected={selected} />}
        </Box>
      </Flex>

      {/* MOBILE MODAL for Details */}
      <Dialog.Root open={isMobileModalOpen}>
        <Portal>
          <Dialog.Backdrop onClick={closeMobileModal} />
          <Dialog.Positioner px={4}>
            ...
            >
              ...
              >
                <CloseButton size="xs" color="#475467" />
              </Dialog.CloseTrigger>
              <Fieldset.Root>
                <Fieldset.Content>
                  {selected && <UserDetails selected={selected} />}
...
const UserDetails = ({ selected }) => (
  <Stack spacing={4}>
    <Card.Root bg="white" shadow="md" rounded={20}>
      <Card.Body gap="2">
        <Avatar.Root mx="auto" boxSize={{ base: "60px", md: "100px" }}>
          <Avatar.Image src={selected?.profile_picture || userImage} />
        </Avatar.Root>
        <Card.Title textAlign="center" fontFamily="InterBold">
          {selected?.first_name} {selected?.last_name}
        </Card.Title>
        <Text textAlign="center" color="#3B3B3B">
          {selected?.profession}
        </Text>
        <Button
          mt={3}
    ...
    <Card.Root shadow="sm" rounded={10}>
      <Card.Header mt={{ base: -4, md: 0 }}>
        <Heading fontSize={{ base: "15px", md: "20px" }}>About Me</Heading>
      </Card.Header>
      <Card.Body mt={{ base: -6, md: 0 }}>
        <Text fontSize={{ base: "11px", md: "16px" }} color="#7C7C7C">
    ...
    <Card.Root shadow="sm" rounded={10}>
      ...
      <Card.Body mt={{ base: -6, md: 0 }}>
        <Text fontSize={{ base: "11px", md: "16px" }} color="#7C7C7C">
          {selected?.years_of_experience || 0} Years of Experience
        </Text>
      </Card.Body>
    </Card.Root>
